...
public class MKLayer: CALayer {
    ...
    public var elevationOffset: CGSize = CGSize() {
        didSet {
            self.enableElevation()
        }
    }
    ...
    private func clearEffects() {
        if let rippleLayer = self.rippleLayer,
            ...
            if rippleEnabled {
                ...
                opacityAnim.delegate = self.delegate as! CAAnimationDelegate?
    ...
    private func startRippleEffect(touchLocation: CGPoint) {
        ...
        if let rippleLayer = self.rippleLayer,
            ...
            scaleAnim.delegate = self.delegate as! CAAnimationDelegate?
